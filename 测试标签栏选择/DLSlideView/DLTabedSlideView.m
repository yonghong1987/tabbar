...
- (void)setCacheCount:(NSInteger)cacheCount{
    _cacheCount = cacheCount;
    if (!ctrlCache_) {
        ctrlCache_ = [[DLLRUCache alloc] initWithCount:cacheCount == 0 ? 1 :cacheCount];
        self.ctrlCache = ctrlCache_;
    }
...
- (UIViewController *)DLSlideView:(DLSlideView *)sender controllerAt:(NSInteger)index{
    ...
    if ([ctrlCache_ objectForKey:key]) {
        self.selectViewController =  [ctrlCache_ objectForKey:key];
        return  self.selectViewController;
    }
    else{
        UIViewController *ctrl = [self.delegate DLTabedSlideView:self controllerAt:index];
        [ctrlCache_ setObject:ctrl forKey:key];
        self.selectViewController = ctrl;
        return self.selectViewController;
    }
}
